...
export class ContentListComponent {
  ...
  addContentToList(newContent: Content) {
		this.ProductService.addContent(newContent).subscribe((newContentFromServer) => {
			this.contentList = [...this.contentList, newContentFromServer];
			this.messageService.add(`New Product ID ${newContentFromServer.id} name ${newContentFromServer.title} added`);
		});
	}
